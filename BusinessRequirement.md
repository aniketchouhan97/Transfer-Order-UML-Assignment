...
Table Facility {
  ...
  facilityTypeId varchar [note: 'RETAIL_STORE or WAREHOUSE']
  facilityName varchar
  primaryFacilityGroupId varchar
}

Table FacilityGroup {
  facilityGroupId varchar [primary key]
  facilityGroupTypeId varchar [note: 'ZONE_MAPPING']
  facilityGroupName varchar
}
...
Table ProductFacility {
  productId varchar [primary key]
  facilityId varchar [primary key]
  minimumStock decimal [note: 'Used as Buffer Level']
  inventoryTurnoverRatio decimal [note: 'Custom field']
}

Table InventoryItem {
  inventoryItemId varchar [primary key]
  productId varchar [not null]
  facilityId varchar [not null]
  lotId varchar
  expireDate date [note: 'Used for FEFO Logic']
  quantityOnHandTotal decimal
  availableToPromiseTotal decimal
}

Table Lot {
  lotId varchar [primary key]
  creationDate date
  expirationDate date
}
...
Table OrderHeader {
  ...
  orderTypeId varchar [note: 'TRANSFER_ORDER']
  originFacilityId varchar [note: 'Sender Location']
  statusId varchar
}
...
Table OrderRole {
  ...
  roleTypeId varchar [primary key, note: 'SHIP_TO_CUSTOMER']
}

Table OrderItem {
  orderId varchar [primary key]
  orderItemSeqId varchar [primary key]
  productId varchar [not null]
  quantity decimal
}
...
Table Shipment {
  ...
  shipmentTypeId varchar [note: 'TRANSFER']
  originFacilityId varchar [not null]
  destinationFacilityId varchar [not null]
  statusId varchar [note: 'SHIPMENT_SHIPPED / IN_TRANSIT']
}

Table ShipmentItem {
  shipmentId varchar [primary key]
  shipmentItemSeqId varchar [primary key]
  productId varchar [not null]
  quantity decimal
}

Table ItemIssuance {
  itemIssuanceId varchar [primary key]
  orderId varchar
  orderItemSeqId varchar
  inventoryItemId varchar
  shipmentId varchar
  shipmentItemSeqId varchar
  quantity decimal
}

// Relationships
Ref: FacilityGroupMember.facilityGroupId > FacilityGroup.facilityGroupId
Ref: FacilityGroupMember.facilityId > Facility.facilityId

Ref: ProductFacility.facilityId > Facility.facilityId
Ref: ProductFacility.productId > Product.productId

Ref: InventoryItem.facilityId > Facility.facilityId
Ref: InventoryItem.productId > Product.productId
Ref: InventoryItem.lotId > Lot.lotId

Ref: OrderHeader.originFacilityId > Facility.facilityId
Ref: OrderRole.orderId > OrderHeader.orderId

Ref: OrderItem.orderId > OrderHeader.orderId
Ref: OrderItem.productId > Product.productId

Ref: Shipment.originFacilityId > Facility.facilityId
Ref: Shipment.destinationFacilityId > Facility.facilityId

Ref: ShipmentItem.shipmentId > Shipment.shipmentId
Ref: ShipmentItem.productId > Product.productId

Ref: ItemIssuance.orderId > OrderHeader.orderId
Ref: ItemIssuance.orderItemSeqId > OrderItem.orderItemSeqId
Ref: ItemIssuance.inventoryItemId > InventoryItem.inventoryItemId
Ref: ItemIssuance.shipmentId > Shipment.shipmentId
Ref: ItemIssuance.shipmentItemSeqId > ShipmentItem.shipmentItemSeqId
```
